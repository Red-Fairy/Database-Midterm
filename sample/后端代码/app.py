...
def register_test():
    ...
    if not data:
        db.session.execute('insert into user(username,password,telephone,role) value("%s","%s","%s",0)' % (
...
def user_unsend():
    if request.method == 'GET':
        ...
        print(phone)
        data = db.session.execute('select * from oorder where checked=0 and cons_phone="%s"' % phone).fetchall()
        Data = []
        for i in range(len(data)):
    ...
    if request.method == 'POST':
        ...
        print(order_id)
        db.session.execute(
            'update oorder set cons_name="%s", cons_addre="%s" where order_id="%d"' % (cons_name, cons_addre, order_id))
        db.session.commit()
        return jsonify(status=200, msg="修改成功")
    if request.method == 'DELETE':
        order_id = request.json.get("delete_id")
        db.session.execute('delete from oorder where order_id="%d" ' % order_id)
        db.session.commit()
...
def user_sending():
    if request.method == 'GET':
        ...
        data = db.session.execute('select * from sending_order where cons_phone="%s"' % phone).fetchall()
        Data = []
        for i in range(len(data)):
...
def user_sended():
    if request.method == 'GET':
        phone = get_token_phone(request.headers.get('token'))
        data = db.session.execute('select * from sended_order where cons_phone="%s"' % phone).fetchall()
        Data = []
        for i in range(len(data)):
            dic = dict(order_id=data[i][0], shop_name=data[i][1], order_money=data[i][2], order_way=data[i][3],
                       cons_phone=data[i][4],
...
def user_pwd_chg():
    if request.method=='POST':
        ...
        phone = get_token_phone(request.headers.get('token'))
        data = db.session.execute('select * from user where telephone="%s" and password="%s"'% (phone,old_pwd)).fetchall()
        if not data:
...
def manager_shop():
    ...
    if request.method == 'POST' and request.json.get('action') == "add":
        ...
        if not exist:
            db.session.execute('insert fastfood_shop(shop_name,price,m_sale_v) value("%s",%d,%d)' % (
...
def manager_dispatcher():
    if request.method == 'GET':
        data = db.session.execute('select * from dispatcher').fetchall()
        Data = []
        for i in range(len(data)):
    ...
    if request.method == 'POST':
        ...
        if not exist:
            db.session.execute(
                'insert dispatcher(dispatcher_id,dispatcher_name,dispatcher_phone) value("%s","%s","%s")' % (
...
def manager_wuliu():
    ...
    if ended == '0':
        data = db.session.execute('select * from wuliu where ended=0').fetchall()
        Data = []
        for i in range(len(data)):
        ...
        data = db.session.execute('select * from wuliu where ended=1').fetchall()
        Data = []
        for i in range(len(data)):
            dic = dict(order_id=data[i][0], cons_phone=data[i][1], disp_id=data[i][2], deliver_time=data[i][3])
            Data.append(dic)
...
def manager_unsend():
    if request.method == 'GET':
        data = db.session.execute('select * from oorder where checked=0').fetchall()
        Data = []
        for i in range(len(data)):
            dic = dict(order_id=data[i][0], shop_name=data[i][1], price=data[i][2], orderway=data[i][3],
                       cons_phone=data[i][4],
    ...
    if request.method == 'POST':
        ...
        db.session.execute('insert wuliu( order_id, cons_phone,disp_id,deliver_time) value(%d,"%s","%s","%s")' % (
...
def manager_sending():
    if request.method == 'GET':
        data = db.session.execute('select * from sending_order').fetchall()
        Data = []
        for i in range(len(data)):
            dic = dict(order_id=data[i][0], shop_name=data[i][1], order_money=data[i][2], order_way=data[i][3],
                       cons_phone=data[i][4],
...
def manager_sended():
    if request.method == 'GET':
        data = db.session.execute('select * from sended_order').fetchall()
        Data = []
        for i in range(len(data)):
            dic = dict(order_id=data[i][0], shop_name=data[i][1], order_money=data[i][2], order_way=data[i][3],
                       cons_phone=data[i][4],
...
if __name__ == '__main__':
    app.run(debug=True, host='192.168.124.5', port='5000')
    # 开启了debug模式
